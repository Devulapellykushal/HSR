...
function SortableProjectItem({ project, statusColors }: { project: any, statusColors: any }) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    ...
    >
      <div className="absolute top-2 right-2 z-20 opacity-0 group-hover:opacity-100 transition-opacity cursor-move" {...attributes} {...listeners}>
        <div className="bg-white p-2 rounded-full shadow-lg border border-gray-200">
          <FiMove className="w-5 h-5 text-gray-600" />
        </div>
      </div>
      {/* Existing Card Content - Simplified for Reorder View */}
...
export default function ProjectsManagement() {
  ...
  const [reorderedProjects, setReorderedProjects] = useState<any[]>([]);

  // Initialize reordered projects when rawProjects changes or entering reorder mode
  useEffect(() => {
    if (rawProjects.length > 0) {
      ...
      // Sort locally by display_order
      mapped.sort((a, b) => a.display_order - b.display_order);
      setReorderedProjects(mapped);
    }
  }, [rawProjects]);
  ...
  const handleSaveOrder = async () => {
    try {
      ...
      await projectsService.reorderProjects(updates);

      setIsReorderMode(false);
      invalidateCache(); // Refresh data
    ...
    <div>
        ...
        <div className="flex gap-2">
          {!isReorderMode ? (
            <button
              ...
              className="w-full sm:w-auto px-4 py-2 bg-white border border-[#2E936B] text-[#2E936B] rounded-lg font-semibold text-sm transition-colors hover:bg-green-50 flex items-center justify-center gap-2"
            >
            ...
            <>
              ...
              >
                {bulkActionLoading ? 'Saving...' : 'Save Order'}
              </button>
              <button
                onClick={() => setIsReorderMode(false)}
                disabled={bulkActionLoading}
                className="w-full sm:w-auto px-4 py-2 bg-gray-200 text-gray-700 rounded-lg font-semibold text-sm transition-colors hover:bg-gray-300"
              >
                Cancel
              </button>
            </>
      ...
      {isReorderMode ? (
        <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
          <p className="mb-4 text-sm text-gray-600">Drag and drop cards to reorder projects.</p>
          <DndContext
            sensors={sensors}
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {reorderedProjects.map((project) => (
                  <SortableProjectItem key={project.id} project={project} statusColors={statusColors} />
                ))}
              </div>
            </SortableContext>
          </DndContext>
