...
export default function AdminLayout({ children }: AdminLayoutProps) {
  ...
  const menuItems = [
    ...
    { id: 'awards', label: 'Awards', icon: FiAward, path: '/admin/awards' },
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#F8F9FA] flex">
        ...
        {/* Sidebar */}
        <aside
          className={`fixed inset-y-0 left-0 z-40 w-64 bg-[#F8F9FA] border-r border-gray-200 transform transition-transform duration-200 ease-in-out md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
            }`}
        >
            ...
            {menuItems.map((item) => {
              ...
              return (
                <Link
                  key={item.id}
                  href={item.path}
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors relative ${active ? 'bg-[#E8F5EF]' : 'hover:bg-gray-50'
                    }`}
                  style={{
